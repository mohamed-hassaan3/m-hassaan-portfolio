...
const Header = () => {
  ...
  return (
    ...
    >
      ...
      <nav>
        <ul className=" flex items-center md:gap-6 gap-1 *:hover:dark:text-neutral-200 sm:*:px-3 *:px-1 text-xs md:text-md font-extrabold tracking-wider">
          {navItems.map((item) => (
            <li key={item.name} className={`relative ${pathname === item.path && "dark:text-neutral-200"}`}>
              <Link href={item.path}>{item.name}</Link>
              {pathname === item.path && (
                <span className="absolute -left-1 top-1 w-2 h-2 bg-[#b5ff6d] rounded-full"></span>
              )}
            </li>
          ))}
        </ul>
